...
class _CreativeUIButtonState extends State<CreativeUIButton> {
  ...
  CreativeUIButtonStyle _effectiveStyle(
    ...
    final defaultPadding =
        inStyle.padding ?? const EdgeInsets.symmetric(vertical: 10, horizontal: 14);
    ...
    switch (variant) {
      ...
      case ButtonVariant.text:
        return inStyle.copyWith(
          ...
          borderWidth: 0,
          borderRadius: inStyle.borderRadius ?? 12,
          padding: inStyle.padding ?? const EdgeInsets.symmetric(vertical: 8, horizontal: 8),
          textStyle: inStyle.textStyle ??
              TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.w700,
  ...
  void _restartAnimationIntervals() {
    ...
    if (pulsing != null) {
      final pause = pulsing.pause ?? const Duration(milliseconds: 100);
  ...
  Widget _withHoveringAnimation(BuildContext context, Widget child) {
    final hasFloat =
        _effective.animationOptions?.animationOptions.containsKey(AnimationType.floating) ?? false;

    if (!hasFloat) return child;

    final cfg = _effective.animationOptions!.animationOptions[AnimationType.floating]!;
    return MirrorAnimationBuilder<double>(
      child: child,
      duration: cfg.duration ?? const Duration(seconds: 1),
      builder: (_, value, c) => Transform.translate(offset: Offset(0, value), child: c),
  ...
  Widget _withPulseAnimation(BuildContext context, Widget child) {
    final hasPulse =
        _effective.animationOptions?.animationOptions.containsKey(AnimationType.pulsing) ?? false;
  ...
  Widget _buildMaterialWrapper(Widget core) {
    ...
    final overlay = _stateOverlayColor(context, variant: _effective.variant ?? ButtonVariant.contained);

    final ink = FocusableActionDetector(
      onShowFocusHighlight: (v) => setState(() => _isFocused = v),
      onShowHoverHighlight: (v) => setState(() => _isHovered = v),
      child: MouseRegion(
        cursor: _effective.disabled ? SystemMouseCursors.basic : SystemMouseCursors.click,
        child: Material(
          ...
          child: InkWell(
            ...
            child: DecoratedBox(
              ...
              decoration: overlay == null ? const BoxDecoration() : BoxDecoration(color: overlay),
              child: core,
            ),
          ),
        ),
  ...
  Color? _stateOverlayColor(BuildContext ctx, {required ButtonVariant variant}) {
    ...
    return opacity == 0.0 ? null : base.withOpacity(opacity);
  }
    ...
    final decoration = BoxDecoration(
      ...
      border: isOutlined
          ...
          : (st.borderColor != null && (st.borderWidth ?? 0) > 0
              ...
              : null),
      boxShadow: st.applyShadow && st.boxShadow != null ? [st.boxShadow!] : null,
    );

    final buttonCore = Container(
      height: st.size?.height,
      width: st.size?.width,
      padding: st.padding ?? const EdgeInsets.symmetric(vertical: 10, horizontal: 14),
  ...
  Widget _buildTextVariant() {
    ...
    return Padding(
      padding: st.padding ?? const EdgeInsets.symmetric(vertical: 8, horizontal: 8),
      child: content,
    );
  }
  ...
  List<Widget> _iconThenLabel({Widget? label, bool allowSpacer = true}) {
    ...
    if (_effective.icon != null &&
        (_effective.label != null || (_effective.labelText?.isNotEmpty ?? false)) &&
